...
public class CollidableBox extends Collidable {
	...
	public CollidableBox(GameObject gameObject, CollisionManager collisionManager) {
	...
	public void collidesWith(Collidable theirs) {
		...
		Transform theirTransform = theirs.getGameObject().getComponent(Transform.class);
		...
		if(ourTransform != null && theirTransform != null) {
			...
			if(collision) {
				logger.info(getGameObject() + " collides with " + theirs.getGameObject());
